...
void AirDeviceId_FPANE_Log(FREContext context, NSString* message) {
    ...
    AirDeviceId_FPANE_DispatchEventWithInfo(context, @"LOGGING", message);
}
...
NSArray* AirDeviceId_FPANE_FREObjectToNSArrayOfUIImage(FREObject object) {
    ...
    for (NSInteger i = 0; i < arrayLength; i++) {
        ...
        UIImage *image = AirDeviceId_FPANE_FREBitmapDataToUIImage(itemRaw);
...
FREObject AirDeviceId_FPANE_CreateError(NSString* error, NSInteger* id) {
    ...
    FREObject argV[] = {
            AirDeviceId_FPANE_NSStringToFREObject(error),
            freId
    };
